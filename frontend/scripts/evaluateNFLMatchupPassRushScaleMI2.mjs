...
} from "../src/engines/matchupIntelligence/NFLMatchupDimensionEngine.js";
...
function buildResearchDimensions(snapshot) {
  const home =
    structuredClone(
  ...
  return buildNFLMatchupDimensions({
    home,
    away,
    weather: null
  });
...
function candidateMatchupEdge({
  ...
  const raw = researchDimensions?.raw ?? {};
...
for (const { snapshot, record } of cohort) {
  ...
  const researchDimensions =
    buildResearchDimensions(snapshot);
