...
async function json(url){ const r=await fetch(url,{headers}); if(!r.ok) throw new Error(`SPORTRADAR_HTTP_${r.status}:${url}`); return await r.json(); }
...
const [depthPayload,txPayload,injuryPayload]=await Promise.all([json(depthUrl),json(txUrl),json(injuryUrl)]);
let signals=[];
...
const adapter=createPostgresResearchRepositoryAdapter({pool,options:{allowSoftDelete:true,allowArchive:true,allowHardDelete:false}}); const service=createNFLAvailabilityResearchRepositoryService({adapter}); const result=await service.persistBundle(bundle); console.log(`Observation writes: ${result.observationWrites}`); console.log(`Artifact writes: ${result.artifactWrites}`); console.log(`Session writes: ${result.sessionWrites}`); console.log(`Unchanged team/week artifacts: ${result.unchangedArtifacts}`); console.log(`Persistence status: ${result.status}`); for(const t of result.teamResults) console.log(`Team ${t.team}: ${t.status}`); if(result.failures.length){ console.error(JSON.stringify(result.failures,null,2)); process.exitCode=1; }
