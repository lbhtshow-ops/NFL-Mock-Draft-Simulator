...
const configuredSupabaseUrl = process.env.RESEARCH_REPOSITORY_SUPABASE_URL || null;
...
function normalizeSupabaseProjectUrl(value) {
  ...
  try {
    ...
    throw new Error("RESEARCH_REPOSITORY_SUPABASE_URL must be a valid HTTPS Supabase project URL.");
  }
  if (parsed.protocol !== "https:") {
    throw new Error("RESEARCH_REPOSITORY_SUPABASE_URL must use HTTPS.");
  }
  // Supabase createClient() expects the project base URL. Operators may copy the
  // Data API endpoint from the dashboard, which ends in /rest/v1. Passing that
  // endpoint directly causes supabase-js to append /rest/v1 a second time and
  // PostgREST rejects the resulting path with PGRST125.
  parsed.pathname = parsed.pathname.replace(/\/rest\/v1\/?$/i, "").replace(/\/+$/, "");
  parsed.search = "";
  parsed.hash = "";
  if (parsed.pathname && parsed.pathname !== "/") {
    throw new Error("RESEARCH_REPOSITORY_SUPABASE_URL must be the Supabase project base URL (optionally copied as the /rest/v1 Data API URL).");
  }
