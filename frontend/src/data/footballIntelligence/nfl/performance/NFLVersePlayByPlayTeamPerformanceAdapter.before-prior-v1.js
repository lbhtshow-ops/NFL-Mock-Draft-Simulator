...
export function aggregateNFLVersePlayByPlay({
  ...
  for (const row of rows) {
    ...
    if (Number.isFinite(rowSeason) && rowSeason !== Number(season)) {
      continue;
    }
    ...
    if (gameId) {
      offense.gameIds.add(String(gameId));
      defense.gameIds.add(String(gameId));

      const phase = normalizeSeasonType(row);

      registerGame(offense, {
        gameId,
        week,
  ...
  return [...aggregateByTeam.values()]
    .map((aggregate) =>
      createNFLTeamPerformanceEvidence({
        ...
        defense: {
          epaAllowedPerPlay: safeRate(
            aggregate.defense.epaAllowed,
            aggregate.defense.plays
          ),
          successRateAllowed: safeRate(
            aggregate.defense.successesAllowed,
            aggregate.defense.plays
          ),
          passEpaAllowedPerPlay: safeRate(
            aggregate.defense.passEpaAllowed,
            aggregate.defense.passPlays
          ),
          rushEpaAllowedPerPlay: safeRate(
            aggregate.defense.rushEpaAllowed,
