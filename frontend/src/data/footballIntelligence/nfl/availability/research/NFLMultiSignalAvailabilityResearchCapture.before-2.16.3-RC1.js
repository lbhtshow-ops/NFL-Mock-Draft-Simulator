...
export function getNFLMultiSignalAvailabilitySessionId(season, week) {
  return `research-session:nfl-multisignal-availability:${Number(season)}:${Number(week)}`;
}
export function getNFLMultiSignalAvailabilityEvidenceId(season, week, team) {
  return `evidence:nfl-multisignal-availability:${Number(season)}:${Number(week)}:${token(team)}`;
}
...
export function createNFLMultiSignalAvailabilityResearchSource({ checkedAt = new Date().toISOString() } = {}) {
  return researchRepository.createResearchSource({
    ...
    access: { type: researchRepository.RESEARCH_SOURCE_ACCESS_TYPES.LICENSED, location: "https://api.sportradar.com/nfl/official/", requiresAuthentication: true },
...
function createObservation(signal, checkedAt) {
  ...
  const observationId = ["observation:nfl-multisignal-availability", signal.season, signal.week, token(signal.team), token(pRef), token(signal.signalClass), fingerprint].join(":");
  return researchRepository.createRecordedObservation({
    observationId, sessionRef: getNFLMultiSignalAvailabilitySessionId(signal.season, signal.week), sourceRefs: [NFL_MULTI_SIGNAL_AVAILABILITY_SOURCE_ID],
    ...
    metadata: { tags: ["nfl-player-availability","multi-signal",`signal:${signal.signalClass}`,`authority:${signal.authority}`,`season:${signal.season}`,`week:${signal.week}`,`team:${signal.team}`,`provider:${signal?.provenance?.source || "unknown"}`], externalRefs: [signal?.provenance?.sourceUrl].filter(Boolean) },
  });
}

function createSession({ season, week, observationRefs, evidenceRefs, checkedAt }) {
  return researchRepository.createResearchSession({
    sessionId: getNFLMultiSignalAvailabilitySessionId(season, week), title: `NFL Multi-Signal Availability Acquisition — ${season} Week ${week}`,
    ...
    provenance: { createdBy: NFL_MULTI_SIGNAL_AVAILABILITY_ACTOR, createdAt: checkedAt, updatedBy: NFL_MULTI_SIGNAL_AVAILABILITY_ACTOR, updatedAt: checkedAt },
    metadata: { tags: ["nfl-multisignal-availability",`season:${season}`,`week:${week}`] },
  });
}

function createArtifact({ season, week, team, signals, observationRefs, checkedAt }) {
  ...
  return researchRepository.createEvidenceArtifact({
    evidenceId: getNFLMultiSignalAvailabilityEvidenceId(season, week, team), sessionRef: getNFLMultiSignalAvailabilitySessionId(season, week), sourceRefs: [NFL_MULTI_SIGNAL_AVAILABILITY_SOURCE_ID],
...
export function createNFLMultiSignalAvailabilityResearchBundle(signals = [], { checkedAt = new Date().toISOString() } = {}) {
  const canonical = (Array.isArray(signals) ? signals : []).filter(s => s?.season && s?.week && s?.team && s?.signalClass);
  const source = createNFLMultiSignalAvailabilityResearchSource({ checkedAt });
  const groups = new Map();
  for (const signal of canonical) {
    const key = `${signal.season}:${signal.week}:${signal.team}`;
  ...
  for (const group of groups.values()) {
    ...
    const artifact = createArtifact({ season, week, team, signals: group, observationRefs: obs.map(o=>o.observationId), checkedAt }); artifacts.push(artifact);
    const key = `${season}:${week}`; const state = sessions.get(key) || { season, week, observationRefs: [], evidenceRefs: [] };
