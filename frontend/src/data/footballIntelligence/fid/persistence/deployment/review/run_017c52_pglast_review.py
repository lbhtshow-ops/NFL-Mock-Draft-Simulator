"""Offline pglast review for Sprint 17C.52; never executes PostgreSQL."""
import json,re
...
print(json.dumps({"tool":f"pglast {pglast.__version__}","grammar":parser.get_postgresql_version(),"file":path.name,"completeStatementTypes":complete,"plpgsqlParsed":bool(plpgsql),"visibleSelectStatementTypes":sorted(set(re.findall(r'"([A-Za-z]+Stmt)"',parser.parse_sql_json(visible)))),"dynamicMetadataStatementTypes":sorted(set(re.findall(r'"([A-Za-z]+Stmt)"',parser.parse_sql_json(dynamic)))),"jsonbBuilders":len(re.findall(r'pg_catalog\.jsonb_build_object\(',sql)),"visibleSelects":len(re.findall(r'^SELECT r\.\* FROM',sql,re.M)),"sqlExecuted":False},indent=2))
