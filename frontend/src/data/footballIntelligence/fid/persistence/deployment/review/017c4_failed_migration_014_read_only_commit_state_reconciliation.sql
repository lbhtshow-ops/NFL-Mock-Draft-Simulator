-- SPRINT 17C.4 FAILED MIGRATION 014 COMMIT-STATE RECONCILIATION. READ-ONLY.
...
SELECT expected.table_name,
       ...
       COALESCE((SELECT COUNT(*)::integer FROM pg_catalog.pg_constraint AS constraint WHERE constraint.conrelid=class.oid),0) AS constraint_count,
...
WITH expected_constraints(table_name,constraint_name,expected_type) AS (
  ...
  SELECT expected.table_name, expected.constraint_name, expected.expected_type,
         constraint.oid, constraint.contype
  ...
  LEFT JOIN pg_catalog.pg_constraint AS constraint
    ON constraint.conrelid=class.oid AND constraint.conname=expected.constraint_name
)
...
WITH expected_tables(table_name,expected_columns) AS (
  ...
  SELECT COUNT(class.oid)::integer AS table_count,
         ...
         COALESCE(SUM((SELECT COUNT(*) FROM pg_catalog.pg_constraint AS constraint WHERE constraint.conrelid=class.oid)),0)::integer AS constraint_count,
         COALESCE(SUM((SELECT COUNT(*) FROM pg_catalog.pg_constraint AS constraint WHERE constraint.conrelid=class.oid AND constraint.conname=ANY(expected_constraint_names.names))),0)::integer AS exact_constraint_count,
         COALESCE(SUM((SELECT COUNT(*) FROM pg_catalog.pg_constraint AS constraint WHERE constraint.conrelid=class.oid AND NOT constraint.conname=ANY(expected_constraint_names.names))),0)::integer AS unexpected_constraint_count,
