-- Sprint 17C.34: exact-target-bound mismatch-detail diagnostic correction.
BEGIN TRANSACTION READ ONLY;

WITH
constants AS (
  SELECT 'Lunch Break Hot Take'::text organization,'LBHT FID Persistence Test'::text project_name,'us-east-1'::text region,
    'ahmorpzcaapvoymiqlkv'::text project_id,'Primary Database'::text database,'main'::text branch,
    'postgres'::text sql_role,'DEDICATED_NON_PRODUCTION_TEST'::text governed_environment,
...
target_binding AS (
  SELECT c.*,(c.organization='Lunch Break Hot Take' AND c.project_name='LBHT FID Persistence Test'
    AND c.region='us-east-1' AND c.project_id='ahmorpzcaapvoymiqlkv' AND c.database='Primary Database'
    AND c.branch='main' AND c.sql_role='postgres' AND c.governed_environment='DEDICATED_NON_PRODUCTION_TEST') binding_valid
  FROM constants c
),
identities AS (
...
SELECT 'EXACT_TARGET_BOUND_FID_FUNCTION_OWNER_CAPABILITY_MISMATCH_DETAIL_RESULT'::text result_identity,'17C.34.1'::text result_version,
  CASE WHEN NOT b.binding_valid THEN 'TARGET_BINDING_INCONSISTENT' WHEN jsonb_array_length(u.unresolved_details)>0 THEN 'EVIDENCE_UNRESOLVED' WHEN s.mismatch_count=0 AND jsonb_array_length(x.state_conflicts)=0 THEN 'NO_MISMATCHES'
    ELSE 'MISMATCH_DETAIL_REVIEW_REQUIRED' END::text classification,s.mismatch_count,s.mismatch_details,
  ...
  c.organization,c.project_name,c.region,c.project_id,c.database,c.branch,c.sql_role,c.governed_environment,b.binding_valid target_binding_valid,
  true read_only,0::integer mutation_count
FROM summary s CROSS JOIN unresolved_summary u CROSS JOIN state_summary x CROSS JOIN state st CROSS JOIN target_binding b CROSS JOIN constants c;
