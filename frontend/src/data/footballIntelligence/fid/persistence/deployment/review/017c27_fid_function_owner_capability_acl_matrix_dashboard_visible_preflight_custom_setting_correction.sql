-- Sprint 17C.27: PostgreSQL-valid custom-setting correction successor.
BEGIN TRANSACTION READ ONLY;

DO $acl_matrix$
DECLARE
  result_setting constant text := 'lbht.preflight_017c24_result';
...
BEGIN
  ...
  IF metadata_oid IS NULL THEN
    PERFORM pg_catalog.set_config(result_setting, pg_catalog.jsonb_build_object(
      ...
      'result_version','17C.27.1','mode',mode,
  ...
  PERFORM pg_catalog.set_config(result_setting, pg_catalog.jsonb_build_object(
    'result_identity','FID_FUNCTION_OWNER_CAPABILITY_PREFLIGHT_VISIBLE_RESULT',
    'result_version','17C.27.1','mode',mode,'classification',classification,
...
WITH payload AS (
  SELECT pg_catalog.current_setting('lbht.preflight_017c24_result', true)::jsonb AS value
)
SELECT
...
WHERE value->>'result_identity'='FID_FUNCTION_OWNER_CAPABILITY_PREFLIGHT_VISIBLE_RESULT'
  AND value->>'result_version'='17C.27.1'
