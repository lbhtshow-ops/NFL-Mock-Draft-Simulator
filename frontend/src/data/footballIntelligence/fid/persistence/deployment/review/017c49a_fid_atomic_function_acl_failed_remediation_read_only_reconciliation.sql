-- Sprint 17C.49 read-only reconciliation. Repository artifact only; execution is not authorized.
BEGIN TRANSACTION READ ONLY;
DO $fid_acl_017c49$
DECLARE
  setting_name constant text := 'lbht.fid_acl_017c49_reconciliation';
  ...
  metadata_ref regclass; metadata_shape_match boolean := false; metadata_count bigint;
  object_count integer; failures jsonb; failed_count integer; identity_complete boolean; acl_evidence_complete boolean;
  properties_exact boolean; roles_exact boolean; before_acl_exact boolean; after_acl_exact boolean;
  rollback_exact boolean; evidence_complete boolean; classification text;
BEGIN
  ...
  IF metadata_ref IS NOT NULL THEN
    SELECT count(*)=3 INTO metadata_shape_match FROM pg_catalog.pg_attribute
    WHERE attrelid=metadata_ref AND attnum>0 AND NOT attisdropped AND attname=ANY(ARRAY['migration_id','migration_sequence','applied_migration_ids']::name[]);
  ...
  rollback_exact:=metadata_ref IS NOT NULL AND metadata_shape_match AND metadata_count=0 AND object_count=0;
  evidence_complete:=identity_complete AND acl_evidence_complete AND metadata_ref IS NOT NULL AND metadata_shape_match AND metadata_count IS NOT NULL AND object_count IS NOT NULL;
  SELECT COALESCE(pg_catalog.jsonb_agg(label ORDER BY ordinal),'[]'::jsonb) INTO failures FROM (VALUES
    ...
    (32,'migration_metadata_available_and_compatible',metadata_ref IS NOT NULL AND metadata_shape_match),(33,'migration_014_metadata_absent',metadata_count=0),(34,'migration_014_objects_absent',object_count=0)
  ) predicates(ordinal,label,passed) WHERE passed IS NOT TRUE;
  failed_count:=pg_catalog.jsonb_array_length(failures);
  classification:=CASE
  ...
  PERFORM pg_catalog.set_config(setting_name,pg_catalog.jsonb_build_object(
    'result_identity','FID_ATOMIC_FUNCTION_ACL_FAILED_REMEDIATION_RECONCILIATION_017C49','result_version','17C.49.1','mode','READ_ONLY_RECONCILIATION','classification',classification,
    'failed_baseline_predicate_count',failed_count,'failed_baseline_predicates',failures,'detail_count_matches_failed_count',pg_catalog.jsonb_array_length(failures)=failed_count,
    ...
    'evidence_complete',evidence_complete,'external_target_attestation_required',true,'postgres_observed_external_target_values',false,'overall_target_verified',false,'read_only',true,'mutation_count',0)::text,true);
END
$fid_acl_017c49$;
SELECT r.* FROM pg_catalog.jsonb_to_record(pg_catalog.current_setting('lbht.fid_acl_017c49_reconciliation',true)::jsonb) AS r(
  result_identity text,result_version text,mode text,classification text,failed_baseline_predicate_count integer,failed_baseline_predicates jsonb,detail_count_matches_failed_count boolean,
