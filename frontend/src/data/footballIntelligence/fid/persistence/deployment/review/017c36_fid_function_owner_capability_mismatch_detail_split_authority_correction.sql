-- Sprint 17C.36: split-authority mismatch-detail target-evidence correction.
...
SELECT 'SPLIT_AUTHORITY_FID_FUNCTION_OWNER_CAPABILITY_MISMATCH_DETAIL_RESULT'::text result_identity,'17C.36.1'::text result_version,
  ...
  jsonb_build_object('authority_source','CANONICAL_PROSPECT_IDENTIFIER_SUPABASE_NON_PRODUCTION_TEST_TARGET_AUTHORIZATION@1.0.0',
    ...
    'organization',c.organization,'project_name',c.project_name,'project_id',c.project_id,'region',c.region,
    'branch',c.branch,'database_source',c.database_source,'governed_environment',c.governed_environment) externally_authorized_target_binding,
...
FROM summary s CROSS JOIN unresolved_summary u CROSS JOIN state_summary x CROSS JOIN state st
  CROSS JOIN constants c CROSS JOIN database_observed_target d;
