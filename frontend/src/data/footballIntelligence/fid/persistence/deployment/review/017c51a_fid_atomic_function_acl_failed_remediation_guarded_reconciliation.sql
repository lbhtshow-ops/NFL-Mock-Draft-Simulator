-- Sprint 17C.51 guarded read-only reconciliation. Execution requires its exact single-use authorization.
BEGIN TRANSACTION READ ONLY;
DO $fid_acl_017c51$
DECLARE
  setting_name constant text := 'lbht.fid_acl_017c51_reconciliation';
...
BEGIN
  ...
  PERFORM pg_catalog.set_config(setting_name,pg_catalog.jsonb_build_object(
    'result_identity','FID_ATOMIC_FUNCTION_ACL_FAILED_REMEDIATION_GUARDED_RECONCILIATION_017C51','result_version','17C.51.1','mode','READ_ONLY_RECONCILIATION','classification',classification,
...
END
$fid_acl_017c51$;
SELECT r.* FROM pg_catalog.jsonb_to_record(pg_catalog.current_setting('lbht.fid_acl_017c51_reconciliation',true)::jsonb) AS r(
