-- SPRINT 17C.7 SUPABASE FUNCTION-OWNER DEPLOYMENT CAPABILITY PREFLIGHT. READ-ONLY.
...
WITH roles AS (
  ...
  SELECT deployment_role_oid,
         ...
         CASE WHEN owner_role_oid IS NULL OR pg_catalog.to_regnamespace('fid') IS NULL THEN false
              ELSE pg_catalog.has_schema_privilege(owner_role_oid,pg_catalog.to_regnamespace('fid'),'CREATE') END AS owner_schema_create
  FROM roles
)
SELECT deployment_role_oid::pg_catalog.regrole AS deployment_role,
       ...
       owner_schema_create,
       (set_capability AND owner_schema_create) AS ownership_transfer_capability,
       ...
       CASE
         ...
         WHEN set_capability AND owner_schema_create THEN 'FUNCTION_OWNER_TRANSFER_CAPABILITY_CONFIRMED'
