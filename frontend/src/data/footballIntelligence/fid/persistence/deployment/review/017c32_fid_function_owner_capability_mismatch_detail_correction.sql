-- Sprint 17C.32: mismatch-detail count, prerequisite, state, membership, and target correction.
BEGIN TRANSACTION READ ONLY;

WITH
constants AS (
  SELECT 'ahmorpzcaapvoymiqlkv'::text project_id,'Primary Database'::text database,'main'::text branch,
...
SELECT 'FID_FUNCTION_OWNER_CAPABILITY_MISMATCH_DETAIL_RESULT'::text result_identity,'17C.32.1'::text result_version,
  CASE WHEN jsonb_array_length(u.unresolved_details)>0 THEN 'EVIDENCE_UNRESOLVED' WHEN s.mismatch_count=0 AND jsonb_array_length(x.state_conflicts)=0 THEN 'NO_MISMATCHES'
  ...
  c.project_id,c.database,c.branch,c.sql_role,c.governed_environment,
  true read_only,0::integer mutation_count
FROM summary s CROSS JOIN unresolved_summary u CROSS JOIN state_summary x CROSS JOIN state st CROSS JOIN constants c;
