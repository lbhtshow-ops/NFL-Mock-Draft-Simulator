...
} from "../../teamIntelligence/dependency/NFLTeamDependencyEvidenceContract.js";
import {
  resolveNFLPlayerTeamDependencyEvidence,
} from "../../teamIntelligence/dependency/NFLPlayerTeamDependencyIntelligence.js";

export const NFL_PLAYER_IMPACT_INPUT_INTEGRATION_VERSION =
  "FIE-NFL-PLAYER-IMPACT-INPUT-INTEGRATION-1.1.0";
...
export function resolveNFLPlayerImpactIntegratedInputs({
  ...
  const replacementPlayerName =
    preliminary?.replacement?.playerName || null;
  ...
  const dependency =
    ...
    resolveNFLPlayerTeamDependencyEvidence({
      ...
      team,
      playerId:
        affected?.caliber?.playerId ||
        affected?.rosterRow?.player_id ||
        null,
      playerName,
      position:
        canonicalAvailabilityPlayer?.role?.depthPosition ||
        canonicalAvailabilityPlayer?.player?.position ||
        affected?.rosterRow?.position ||
        null,
      usageEvidence: usage,
    });
