...
export function evaluateCalibratedPlayerAvailabilityImpact({
  ...
  if (!validation.valid) {
    return unavailable(
      "No approved Player Availability Impact methodology profile is available.",
      validation.errors,
    );
  }
  ...
  return {
    modelState: PLAYER_IMPACT_MODEL_STATES.MODELED,
    overallImpact,
    ...
    featureVector,
    explanation: [
      "Impact was calculated from an explicitly approved calibration profile.",
      ...
      "Availability status influences the result only through the approved profile's calibrated availability multiplier.",
    ],
    limitations: [
