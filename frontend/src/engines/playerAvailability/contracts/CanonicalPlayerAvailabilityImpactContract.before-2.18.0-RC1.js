...
export function createCanonicalPlayerAvailabilityImpactResult({
  ...
  return {
    ...
    impact: {
      modelState,
      overallImpact: modelState === PLAYER_IMPACT_MODEL_STATES.MODELED
        ? impact?.overallImpact ?? null
        : null,
      offensiveImpact: modelState === PLAYER_IMPACT_MODEL_STATES.MODELED
        ? impact?.offensiveImpact ?? null
        : null,
      defensiveImpact: modelState === PLAYER_IMPACT_MODEL_STATES.MODELED
        ? impact?.defensiveImpact ?? null
        : null,
      specialTeamsImpact: modelState === PLAYER_IMPACT_MODEL_STATES.MODELED
        ? impact?.specialTeamsImpact ?? null
        : null,
      confidence: modelState === PLAYER_IMPACT_MODEL_STATES.MODELED
...
export function validateCanonicalPlayerAvailabilityImpactResult(value) {
  ...
  if (value.impact?.modelState === PLAYER_IMPACT_MODEL_STATES.MODELED) {
    if (
      ...
      errors.push("MODELED_OVERALL_IMPACT_MUST_BE_BOUNDED");
    }
    if (!value.impact?.methodology?.profileId || !value.impact?.methodology?.profileVersion) {
      errors.push("MODELED_IMPACT_REQUIRES_METHODOLOGY_IDENTITY");
    }
  }
