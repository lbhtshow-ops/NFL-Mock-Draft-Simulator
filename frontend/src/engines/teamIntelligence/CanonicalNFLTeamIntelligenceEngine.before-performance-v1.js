...
export function getNFLTeamIntelligenceResult(team) {
  const input =
    createNFLTeamIntelligenceInput(team);
  ...
  return createNFLTeamIntelligenceResult({
    ...
    // Sprint 1 intentionally does not calculate a strength score.
    // The audited repository does not yet contain approved canonical
    // NFL performance, opponent-adjustment, or player-availability
    // providers sufficient to support one without guessing.
    ...
    summary:
      state === NFL_TEAM_INTELLIGENCE_STATES.UNAVAILABLE
        ...
        : `NFL Team Intelligence foundation is available for ${input.teamAbbreviation}, but canonical team-strength scoring remains intentionally unmodeled until governed performance and availability evidence providers are connected.`,
    ...
    explanation: {
      ...
      limitingFactors: [
        ...
        "Opponent-adjusted performance evidence is not yet connected.",
      ],
      contextualFactors,
    },
