...
} from "../../../data/footballIntelligence/nfl/performance/NFLTeamPerformanceEvidenceContract.js";
...
export const NFL_PLAYER_TEAM_DEPENDENCY_METHODOLOGY = Object.freeze({
  ...
  limitations: Object.freeze([
    ...
    "Team-performance indexes are attached as contextual evidence and confidence support; they are not a weighted dependency-score component in V1.",
  ]),
});
...
export function resolveNFLPlayerTeamDependencyEvidence({
  ...
  const teamPerformanceContractValid =
    Boolean(
      teamPerformance &&
      isNFLTeamPerformanceEvidence(teamPerformance)
    );

  const performanceIndexes =
    teamPerformanceContractValid
      ? buildNFLPerformanceIndexes(
          teamPerformance,
          peerPerformance
        )
  ...
  const confidence =
    confidenceFor({
      ...
      teamGames,
      componentCount,
      performanceAvailable: teamPerformanceContractValid,
      seasonAligned:
        Number(usageEvidence?.season) === evidenceSeason,
    });
  ...
  return createNFLTeamDependencyEvidence({
    ...
    evidenceRefs: [
      `nflverse-player-stats:${evidenceSeason}:${code}:${playerId || normalizeName(playerName)}`,
      ...(usageEvidence
        ? [`nflverse-snap-counts:${Number(usageEvidence.season)}:${code}:${playerId || normalizeName(playerName)}`]
        : []),
      ...(teamPerformanceContractValid
        ? [`nfl-team-performance:${evidenceSeason}:${code}`]
        : []),
      ...(teamPerformance?.provenance?.sourceUrl
        ? [teamPerformance.provenance.sourceUrl]
        : []),
    ],
    observedAt:
      teamPerformance?.provenance?.generatedAt ||
      usageEvidence?.provenance?.generatedAt ||
      null,
    methodology: {
    ...
    components: {
      ...
      teamPerformanceUsedInDependencyScore: false,
    },
    sample: {
      evidenceSeason,
      playerGames,
      ...
      teamPerformanceContract:
        teamPerformance?.contract || null,
      teamPerformanceVersion:
        teamPerformance?.version || null,
    },
    evidenceState: {
      playerStatsAvailable: playerRows.length > 0,
      usageEvidenceAvailable: Boolean(usageEvidence),
      teamPerformanceRecordAvailable: Boolean(teamPerformance),
      teamPerformanceContractValid,
      teamPerformanceIndexesAvailable:
        finite(performanceIndexes?.offense) ||
        finite(performanceIndexes?.recentForm),
    },
  });
}
