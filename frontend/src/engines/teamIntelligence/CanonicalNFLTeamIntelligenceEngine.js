...
function availabilityEvidence(input) {
  const availability = input?.availability;
  if (!availability?.available) return null;
  return {
    type: "NFL_PLAYER_AVAILABILITY_IMPACT",
...
function teamProfileEvidence(input) {
  if (!input?.teamProfile?.available) return null;
  return {
    type: "NFL_TEAM_PROFILE",
    contract: input.teamProfile.contract,
    contractVersion: input.teamProfile.contractVersion,
    rosterState: input.teamProfile.rosterState,
    units: input.teamProfile.units,
    positionGroups: input.teamProfile.positionGroups,
    scoring: input.teamProfile.scoring,
  };
}
...
export function getNFLTeamIntelligenceResult(team, options = {}) {
  ...
  if (input?.teamProfile?.available) {
    contextualFactors.push(
      "Canonical roster-state, position-group, and unit synthesis is available; team-strength scoring remains unmodeled."
    );
  }

  if (input?.availability?.available) {
    contextualFactors.push(
      `Canonical player availability evidence is connected for ${input.availability.playerCount} player(s); team-strength scoring remains unmodeled.`
    );
  }
  ...
  return createNFLTeamIntelligenceResult({
    ...
    teamProfile: input.teamProfile || null,
